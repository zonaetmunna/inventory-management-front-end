...
const ViewProfile = ({
	...
	const personalInformation = personalDataArray.map((item) => ({
		heading: item[0],
		content: item[1],
	}));

	const officialInformation = officialDataArray.map((item) => ({
		heading: item[0],
		content: item[1],
	}));
				...
				<div className="col-span-9">
					{personalInformation.map(({ heading, content }) => (
						<div className="grid grid-cols-12 my-4">
							<div className="col-span-6 font-semibold">{heading}</div>
							<div className="col-span-6 capitalize">:{content}</div>
						</div>
					))}
				</div>
