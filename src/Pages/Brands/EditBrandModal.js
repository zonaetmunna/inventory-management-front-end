...
const EditBrandModal = ({ isEditModalOpen, setIsEditModalOpen, item, setIsLoading }) => {
	const { name, email, description, website, address, status } = item;
	const defaultValues = {
		name,
		email,
		description,
							...
							<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
								...
								<form onSubmit={handleSubmit(onSubmit)}>
									...
									<div className="w-full">
										...
										/>
										<span className="text-green-600">{errors?.emai && errors?.email?.message}</span>
									</div>
									<div className="w-full">
										<label className="label">
									...
									<div className="w-full">
										...
										<select
											{...register('status')}
											className="select select-bordered w-full bg-white focus:outline-offset-0 focus:outline-black "
										>
