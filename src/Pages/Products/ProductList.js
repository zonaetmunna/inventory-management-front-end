...
import { AiFillDelete, AiFillEye, AiOutlineEdit, AiOutlineSearch } from 'react-icons/ai';
...
const ProductList = () => {
	...
	const tableHeader = ['No', '', 'Name', 'Category', 'Brand', 'Unit'];
		...
		<div className="">
			...
			<div className="mt-10 mx-10 px-10 py-5 rounded bg-white">
              ...
              focus:outline-offset-0 focus:outline-black w-full"
					...
					<div>
						<button onClick={handleAddNavigate} type="button" className="btn btn-primary btn-wide">
							Add Product
						</button>
					</div>
				</div>
				...
				<Table header={tableHeader} totalPage={totalPage} page={page} setPage={setPage}>
					{displayData?.map((item, idx) => (
						<tr key={item?._id}>
							...
							<td className="capitalize">{item?.brandName}</td>
							<td>
