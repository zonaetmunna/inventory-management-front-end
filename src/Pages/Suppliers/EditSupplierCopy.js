...
const EditSupplier = () => {
	...
	const fetchSupplierData = async () => {
		...
		const { gender, dateOfBirth, brandName, branchName, status } = data.data;
		...
		<div className="">
			...
			<div className="mt-10 mx-10 px-10 py-5 rounded bg-white">
              ...
              focus:outline-offset-0 focus:outline-black "
							...
							/>
							<span className="text-green-600">{errors?.name && errors?.name?.message}</span>
						</div>
						<div className="w-full">
							<label className="label">
              ...
              focus:outline-offset-0 focus:outline-black "
							...
							/>
							<span className="text-green-600">{errors?.email && errors?.email?.message}</span>
						</div>
						<div className="form-control w-full">
							...
							<Controller
								...
								render={({ field }) => (
									<ReactSelect isClearable {...field} options={genderList} styles={selectStyle} />
								)}
							/>
							<span className="text-green-600">{errors?.gender && errors?.gender?.message}</span>
						</div>
						<div className="form-control w-full">
							<label className="label">
              ...
              focus:outline-offset-0 focus:outline-black "
						...
						<div className="form-control w-full">
							...
							<Controller
								...
								control={control}
								render={({ field }) => (
									<ReactSelect isClearable {...field} options={brandList} styles={selectStyle} />
								)}
							/>
							<span className="text-green-600">
						...
						<div className="form-control w-full">
							...
							<Controller
								...
								control={control}
								render={({ field }) => (
									<ReactSelect isClearable {...field} options={branchList} styles={selectStyle} />
								)}
							/>
							<span className="text-green-600">
                ...
                {errors?.inationalIdImageURL && errors?.nationalIdImageURL?.message}
						...
						<div className="form-control w-full">
							...
							<Controller
								...
								control={control}
								render={({ field }) => (
									<ReactSelect isClearable {...field} options={statusList} styles={selectStyle} />
								)}
							/>
							<span className="text-green-600">{errors?.status && errors?.status?.message}</span>
						</div>
					</div>

					<div>
						<button type="submit" className="btn btn-block btn-primary my-10">
							ADD
						</button>
					</div>
				</form>
